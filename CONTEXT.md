...
- **Tool Surface Readiness**: the Tool Contract module's runtime face — verification at SDK session init that the live tool surface (registered tools + MCP server statuses) covers the Unit's required workflow tools, aborting before the first model turn only on conditions that cannot self-heal: server `failed`/`needs-auth`/`disabled`, server absent from the init surface, or a connected server missing required tools. A `pending` (still-connecting) server passes through — the SDK does not wait for MCP connect before init, so `pending` is the common healthy case and the server typically registers before the Unit's first workflow tool call; a genuine miss then classifies in-session as `tool-unavailable`. Complements the static pre-dispatch gate (`getWorkflowTransportSupportError`), which only proves the MCP launch config is discoverable. See `docs/dev/ADR-036-tool-surface-readiness.md`.
...
- **Single Writer Layer**: the `db/writers/` directory whose files collectively hold every write-SQL statement against the engine DB. The structural invariant is enforced on this directory, not on a single filename. Each file is one cohesive write subsystem (hierarchy, memory, gates, escalation, worktree reconcile, manifest restore, legacy import, cascades).
...
- The Single Writer should be a directory layer, not a single file. `gsd-db.ts` (3,644 lines, 132 exports) explodes into:
  ...
  - `db/writers/*.ts` — one cohesive write subsystem per file (hierarchy, memory, gates, escalation, worktree-reconcile, manifest, legacy-import, cascades). Collectively the **Single Writer Layer**.
