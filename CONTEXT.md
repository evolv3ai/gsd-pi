...
- **Unit Closeout module**: the module (`unit-closeout.ts`) that owns the durable completion pipeline for a Unit behind one interface, `closeUnit(request)` — idempotent per `(basePath, unitId, boundary, outcome)`. Dispatch, retry policy, and Recovery decisions stay outside; `closeUnit` reports typed results. Today it carries the Interactive Closeout adapter's durable git subset; re-seating the auto pipeline behind it is the recorded next step. See `docs/dev/ADR-032-unit-closeout-seam.md`.
...
- **Interactive Closeout adapter**: the adapter at the Unit Closeout seam for non-auto sessions. Attaches at the host's `tool_result` observation hook on the closeout tools, is a no-op while `isAutoActive()`, and runs the durable git subset (commit + Closeout Git Verdict). Exists so interactive completion stops silently bypassing `git.isolation`.
