...
- **Unit Registry**: the single declarative table (`unit-registry.ts`) mapping each Unit type to its **Unit Descriptor**. The source from which `KNOWN_UNIT_TYPES`/`UnitType`, the tool contracts, the scope-class Sets, direct one-template prompt associations (`promptTemplate`), and the unit→phase chain are derived. Conditional/composite prompt selection still lives in `auto-prompts.ts`. Preserves the pre-registry asymmetries explicitly: `discuss-slice`/`execute-task-simple` are `kind: "variant"` (contracts and scope Sets, but excluded from `KNOWN_UNIT_TYPES`); `triage-captures`/`quick-task` carry `toolContract: null` and `phaseChain: null`. See `docs/dev/ADR-033-unit-type-registry.md`.
- **Unit Descriptor**: one Unit type's declaration — kind (primary/variant), scope class (`execute-task` / `section-close` / `standard`), Phase routing chain, direct prompt-template id when the unit has one, and tool surface contract. Prompt *composition* and conditional/composite template selection stay in `auto-prompts.ts`; the descriptor only declares verified direct associations.
