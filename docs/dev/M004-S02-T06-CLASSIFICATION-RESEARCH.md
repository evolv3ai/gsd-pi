...
The retained-byte action-matrix seam now proves that the GSD interpreter's real decision candidates and `/decisions` completeness anchor classify against a read-only extraction of the fixture's v44 database as one create, one update, one delete, one preserve, and an omitted D004 no-op (`src/resources/extensions/gsd/tests/legacy-import-preview.test.ts`). This closes the earlier risk that only hand-built classifier candidates matched the oracle.
